...
def height_converter(x):
    ...
    if pd.isna(x):
        ...
        split_x = x.split('ft')
        height = int(split_x[0])*12
        if len(split_x) == 2:
            if split_x[1] == '':
                pass
            else:
                height = height+int(split_x[1].split('in')[0])
...
p_c = i_p_df.merge(c, left_on='color_id', right_on='id', how='inner')
pcs_df = p_c.merge(iv_df, left_on='inventory_id', right_on='id', how='inner')
p_df = pcs_df.merge(s, left_on='set_num', right_on='set_num', how='inner')
data = pd.pivot_table(data=p_df, values='rgb', index='year', aggfunc="nunique")
plt.plot(data)
